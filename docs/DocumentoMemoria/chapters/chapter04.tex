...
$\hat{\theta}$ toma el valor del presupuesto de emisiones (CAP) impuesto por el gobierno. En esta misma restricción, se utiliza el valor $error$, el cuál representa el error de cálculo de este presupuesto, estimado por \citeB{andres_new_2014}. Finalmente, se define un $error=0.202$. 
\vspace{2.5mm}
...
\caption{{\footnotesize Rendimiento por CAP \textit{Profit Oriented} (Fuente: realización propia)}}
\label{rendcap}
\end{figure}
...
\caption{{\footnotesize Promedio de precios de electricidad (2019-2050)  por escenario (Fuente: realización propia)}}
\label{pidcap}
\end{figure}
...
\caption{{\footnotesize Precios de electricidad (2019-2050)  por escenario (Fuente: realización propia)}}
\label{TCrendcap}
\end{figure}
...
En este modelo el cociente $\frac{\theta}{CAP}$, graficado en la Figura \ref{rendcapMP2}, disminuye mientras mayor es el CAP. Emitiendo cantidades similares a los CAP ($\frac{\theta}{CAP} = 1$). Lo anterior sucede ya que el subastador no invierte en información, entonces este produce cercano al CAP propuesto.
\vspace{2.5mm}
...
Por un lado, es importante mencionar que no se consideraron gastos para el subastador en este modelo ya que su función objetivo solo consideraba un costo llamado Costo Social del Carbón. El anterior fue despreciado en la solución del modelo de equilibrio ya que se consideró independiente de los permisos emitidos ($\theta$) por los autores originales. 
\vspace{2.5mm}

Por otro lado, los modelos reformulados toman los valores de los costos de información adicional $c$ por medio de la calibración del ellos con el original. Por lo que tomar en cuenta las utilidades del subastador para estos modelos tampoco es el indicador más importante a considerar. En un trabajo posterior, se espera calcular estos costos de forma real. 
\vspace{2.5mm}
...
\caption{{\footnotesize Precio de venta de permisos $\pi^a$ por CAP (Fuente: realización propia)}}
\label{piapormodelo}
\end{figure}
...
\caption{{\footnotesize Permisos emitidos por CAP (Fuente: realización propia)}}
...
\caption{{\footnotesize Promedio precios de electricidad (2019-2050) $\pi^d$ por CAP (Fuente: realización propia)}}
\label{pidpormodelo}
\end{figure}
